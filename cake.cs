...
Setup<BuildData>(
    static context =>
    {
        ...
        var projectPath = projectRoot.CombineWithFilePath("pfmcp/pfmcp.csproj");
        ...
        return new BuildData(
            ...
            new DotNetMSBuildSettings()
                ...
                .WithProperty("PackageTags", "tool;mcp;pagefind;search;documentation")
                .WithProperty("PackageDescription", "Pagefind MCP .NET Tool - Search Pagefind indexes and expose them to agents over MCP stdio.")
                .WithProperty("RepositoryUrl", "https://github.com/devlead/pfmcp.git")
...
.Then("Integration-Tests-Tool-Inspect")
    .Does<BuildData>(
        static (context, data) => context.DotNetTool(
            "tool",
            new DotNetToolSettings
            {
                ArgumentCustomization = args => args
                    ...
                    .Append("pfmcp")
                    .Append("inspect")
                    .AppendSwitchQuoted("--index", "https://www.devlead.se/pagefind/pagefind-entry.json"),
                WorkingDirectory = data.IntegrationTestPath
            })
    )
.Then("Integration-Tests-Tool-Search")
    .Does<BuildData>(
        static (context, data) => context.DotNetTool(
            "tool",
            new DotNetToolSettings
            {
                ArgumentCustomization = args => args
                    ...
                    .Append("pfmcp")
                    .Append("search")
                    .AppendSwitchQuoted("--index", "https://www.devlead.se/pagefind/")
                    .Append("cake"),
                WorkingDirectory = data.IntegrationTestPath
            })
    )
